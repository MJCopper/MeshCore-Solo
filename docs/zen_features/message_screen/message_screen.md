...
- **Login has a visible wait and timeout.** If the room does not answer, the password prompt opens so you can retry or replace the password. A timeout does not erase a remembered password because silence can also mean packet loss or an out-of-range server; an explicit rejection does invalidate the rejected saved password.
- **Blank passwords use ACL access.** If an ACL-only login receives no reply, Zen opens the room provisionally after the normal response window. The room server remains authoritative and still rejects messages when that device identity is not permitted by its ACL.
- **Only one on-device login runs at a time.** Its result is returned to the screen that started it, even if another screen is visible when the reply arrives. Phone app and USB login commands retain their normal MeshCore behaviour.
