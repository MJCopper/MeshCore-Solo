...
#include <set>
#include <string>
#include "../../examples/companion_radio/solo/CliCompleter.h"

static std::string digits(const char* word) {
  std::string out;
  while (*word) {
    char digit = solo::T9Predictor::digitFor(*word++);
    if (digit) out += digit;
  }
  return out;
}
...
TEST(CliCompleter, PrioritisesCommandsOverRecentlyUsedChat) {
  char out[8][solo::WordCompleter::MAX_WORD_LEN];
  solo::T9Predictor::remember("pet");
  ASSERT_GT(solo::CliCompleter::suggestT9("738", 3, out, 8), 0);
  EXPECT_STREQ(out[0], "set");
  auto sequence = digits("radio");
  ASSERT_GT(solo::CliCompleter::suggestT9(sequence.c_str(), sequence.size(), out, 8), 0);
...
TEST(CliCompleter, FallsBackToChatWithoutDuplicatingCommands) {
  ...
  uint8_t n = solo::CliCompleter::suggestT9("438", 3, out, 8);
...
TEST(CliCompleter, PreservesDottedKeysAndRespectsByteCapacity) {
  ...
  EXPECT_EQ(solo::CliCompleter::suggestT9(sequence.c_str(), sequence.size(), out, 8, 10), 0);
  ASSERT_GT(solo::CliCompleter::suggest("flood.ad", 8, out, 8), 0);
  EXPECT_STREQ(out[0], "flood.advert.interval");
}

TEST(CliCompleter, TokenRangeIncludesDotsButNotNeighbouringArguments) {
  const char* text = "set flood.advert.interval 3";
  auto range = solo::CliCompleter::currentToken(text, strlen(text), 12);
...
TEST(CliCompleter, PrefixCompletionPrefersCliAndRetainsChatFallback) {
  char out[8][solo::WordCompleter::MAX_WORD_LEN];
  ASSERT_GT(solo::CliCompleter::suggest("ra", 2, out, 8), 0);
  EXPECT_STREQ(out[0], "radio");
  uint8_t n = solo::CliCompleter::suggest("hell", 4, out, 8);
  bool hello = false;
  for (uint8_t i = 0; i < n; i++) if (!strcmp(out[i], "hello")) hello = true;
  EXPECT_TRUE(hello);
  EXPECT_EQ(solo::CliCompleter::suggest("", 0, out, 8), 0);
}

TEST(CliCompleter, DictionaryIsUniqueAndFitsEveryCompletionBuffer) {
  std::set<std::string> unique;
  EXPECT_GT(solo::CliCompleter::dictionarySize(), 80u);
  for (size_t i = 0; i < solo::CliCompleter::dictionarySize(); i++) {
    const char* word = solo::CliCompleter::wordAt(i);
    EXPECT_LT(strlen(word), solo::WordCompleter::MAX_WORD_LEN);
