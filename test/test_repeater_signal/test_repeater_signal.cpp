...
#include "../../examples/companion_radio/solo/RepeaterSignalMonitor.h"

TEST(RepeaterSignal, AcceptsRelayedTrafficButNotDirectCompanions) {
  EXPECT_FALSE(solo::RepeaterSignalMonitor::qualifiesRoute(true, false, 0));
  EXPECT_TRUE(solo::RepeaterSignalMonitor::qualifiesRoute(true, false, 1));
  EXPECT_FALSE(solo::RepeaterSignalMonitor::qualifiesRoute(false, true, 1));
  EXPECT_TRUE(solo::RepeaterSignalMonitor::qualifiesRoute(false, true, 2));
  EXPECT_FALSE(solo::RepeaterSignalMonitor::qualifiesRoute(false, false, 4));
}

TEST(RepeaterSignal, ClassifiesLinkMarginAndAveragesSamples) {
  solo::RepeaterSignalMonitor signal;
